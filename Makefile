...
build-all:
	...
	tar -czvf ${ARTIFACT_ROOT}/$(BINARY)-linux-amd64-$(VERSION).tar.gz Attribution.txt LICENSE res/configuration.toml -C ${ARTIFACT_ROOT} $(BINARY)-linux-amd64
	tar -czvf ${ARTIFACT_ROOT}/$(BINARY)-linux-arm64-$(VERSION).tar.gz Attribution.txt LICENSE res/configuration.toml -C ${ARTIFACT_ROOT} $(BINARY)-linux-arm64
	tar -czvf ${ARTIFACT_ROOT}/$(BINARY)-mac-$(VERSION).tar.gz Attribution.txt LICENSE res/configuration.toml -C ${ARTIFACT_ROOT} $(BINARY)-mac
	zip -j ${ARTIFACT_ROOT}/$(BINARY)-win-$(VERSION).zip ${ARTIFACT_ROOT}/$(BINARY)-win.exe Attribution.txt LICENSE res/configuration.toml
