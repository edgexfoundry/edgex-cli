...
func eventHandler(cmd *cobra.Command, args []string) (err error) {
	...
	if age != "" {
		return deleteOld(cmd, dc)
	} else if len(args) != 0 {
		return deleteByIds(cmd, dc, args)
	}
	return deleteForDevice(cmd, dc)
}

func deleteOld(cmd *cobra.Command, dc coredata.EventClient) error {
	...
	err = dc.DeleteOld(cmd.Context(), int(ageMilliseconds))
	if err == nil {
...
func deleteForDevice(cmd *cobra.Command, dc coredata.EventClient) (err error) {
	err = dc.DeleteForDevice(cmd.Context(), device)
	if err == nil {
...
func deleteByIds(cmd *cobra.Command, dc coredata.EventClient, eventIds []string) error {
	for _, eventId := range eventIds {
		err := dc.Delete(cmd.Context(), eventId)
		if err == nil {
